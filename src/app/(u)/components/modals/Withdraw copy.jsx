...
const Withdraw = ({ userData, setRefresh, setCloseModal,controlSent }) => {
  ...
  const handleTransaction = async (transaction_type) => {
    ...
    if (requiredFields.every((field) => form.values[field] !== "")) {
      try {
        let url =controlSent == true ? `/ctr/${transaction_type === "deposit" ? "deposit_sc_ctl" : "withdraw_sc_ctl"}`:  `/ctr/${transaction_type === "deposit" ? "deposit" : "withdraw"}`
