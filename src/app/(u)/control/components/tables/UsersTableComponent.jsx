...
const UsersTableComponent = ({ refresh, setRefresh, userType }) => {
    ...
    try {
      ...
      if (token) {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/users/control/${userType}`,
          {
            headers: {
  ...
  return (
    ...
    <div className="relative overflow-x-auto shadow-md rounded">
      <p className="text-sm font-bold tracking-wide text-gray-700 my-3">{userType === "normals" ? "Normal" : userType === "panels" ? "Panel" : userType === "admins" ? "Admin" : userType === "masters" ? "Master" : userType === "mains" && "Main Admin"}</p>
