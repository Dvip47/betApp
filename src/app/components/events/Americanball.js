...
import AmericanFootballOddDisplay from "./components/AmericanFootballOddDisplay";

export default function AmericanFootball() {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/events/americanball`
      );
      if (res) {
  ...
  return (
    <div className="">
      <div className="grid grid-col-1 bg-white gap-1">
        {/* top bar */}
        {games.length > 0 && (
          <div className="my-1">
