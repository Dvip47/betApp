...
import BasketOddDisplay from "./components/BasketOddDisplay";
...
export default function Basketball() {
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/events/basketball`
      );
      if (res) {
  ...
  return (
    <div className="">
      <div className="grid grid-cols-1 bg-white gap-1 rounded">
        {/* top bar */}
        {games.length > 0 && (
          <div className="my-1">
