...
import IceHockeyOddDisplay from "./components/IceHockeyOddDisplay";
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/events/icehockey`
      );
      if (res) {
      ...
      <div className="grid grid-col-1 rounded">
        ...
        {loading ? (
          ...
          games.map((game, gameIndex) => {
            if (
              game.event_odds.odds.game_lines &&
              game.event_odds.odds.game_lines.odds.length === 6
            ) {
              return (
                <div key={gameIndex}>
                  <IceHockeyOddDisplay
