...
import RugbyOddsDisplay from "./components/RugbyOddsDisplay";
  ...
  const fetchEvents = async () => {
    try {
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/events/rugby`
      );
      if (res) {
      ...
      <div className="grid grid-col-1">
        ...
        {loading ? (
          ...
          games.map((game, gameIndex) => {
            if (
              game.event_odds.odds.game_betting_3_way &&
              game.event_odds.odds.game_betting_3_way.odds.length === 9
            ) {
              return (
                <RugbyOddsDisplay
                  key={gameIndex}
